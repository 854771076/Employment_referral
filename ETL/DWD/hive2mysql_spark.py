...
class settings:
    ...
    TABLES = [
        ...
        {"name": "企业基本信息", "db": "jobfree", "table": "company", "db_num": 0, "t_num": 0,
         'hive_db': 'dwd_jobfree', 'hive_table': 'dwd_jobfree_db_spider_t_company', "update_column":None},
        {"name": "热门岗位TOP20", "db": "jobfree", "table": "hotjobs_TOP20", "db_num": 0, "t_num": 0,
         'hive_db': 'ads_jobfree', 'hive_table': 'ads_jobfree_db_jobfree_t_hotjobs_TOP20', "update_column":None},
    ]
...
def hive_to_mysql(name, log_data: dict, table: dict, Type):
    ...
    try:
        ...
        if TYPE == 'all':
            ...
            df = df.where(f"{table['update_column']} between '{start_time}' and '{end_time}'").drop('partition_date')
        # 查询为空
        if df.rdd.isEmpty():
